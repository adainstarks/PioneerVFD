...
**Latest Build** - v4.0.1 marketplace build, right-click now-playing context menu, LCD divider with clickable play/pause glyph in the meta track, smoother Ever Scroll cycle (ONCE returns instead of snapping back, end-guard so the final character clears the clip edge on ONCE/BOUNCE), classic Pioneer logo font (Musieer) option, italic hardware silk labels with smaller W&times;4, and contrast polish for snackbar popups, marketplace install/uninstall buttons, and add-to-playlist scrollbars.
