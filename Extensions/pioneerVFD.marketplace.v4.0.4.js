...
(function PioneerVFD() {
  ...
  function openNowPlayingView() {
    ...
    const npv = chassis && chassis.querySelector("[data-pvfd='npv']");
    if (npv) {
      npv.classList.add("active");
      setTimeout(() => npv.classList.remove("active"), 850);
    }
...
//no synthetic fallback
  ...
  // Fullscreen-library detection (issue #18, comment 2): when Your Library is
  // expanded to fullscreen, Spotify's overlay sits at grid z ~4-5 while PVFD
  // pins the chassis-chrome topbar at z 18 — the bar covered the library's
  // create/expand buttons. Geometry check (nav-bar ≈ window width) instead of
  // aria-label sniffing so it survives locale changes. CSS consumes
  // html.pvfd-library-fullscreen to drop the topbar below the overlay.
  function reconcileLibraryFullscreenState() {
    const nav = document.querySelector(".Root__nav-bar, nav[aria-label='Main']");
    const fullscreen = !!nav && nav.getBoundingClientRect().width >= window.innerWidth * 0.95;
