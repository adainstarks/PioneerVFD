...
(function PioneerVFD() {
  ...
  const OEL_WEBM_CLIPS = [
    ...
    // driven by the (future) PULSE audio engine: "shown but locked" until PULSE
  ...
  // Where we were before opening an external lyrics provider, so the button
  // can close back to it. Snapshotted case-preserved (Spotify entity IDs are
  // case-sensitive base62). Only Spicy uses it today.
  let preLyricsPath = "";
  ...
  function openLyrics() {
    ...
    const nativeLyricsOpen = safeReturn(() =>
      currentSpotifyPath().includes("/lyrics") ||
      !!document.querySelector("button[aria-label='Hide lyrics'], button[aria-label='Hide Lyrics']"),
      false);
    if (!nativeLyricsOpen) playStateFlash("lyrics");
  ...
  // ----- ADAPTIVE TINT (safe first-cut engine) --------------------------
