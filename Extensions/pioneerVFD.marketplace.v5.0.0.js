...
(function PioneerVFD() {
  ...
  function ensureOelVideoMarkup() {
    ...
    pulseLock.setAttribute("aria-hidden", "true");
    pulseLock.setAttribute("aria-label", "Enable PULSE");
    pulseLock.setAttribute("role", "button");
    pulseLock.setAttribute("tabindex", "-1");
  ...
  function syncOelColorModeAttributes() {
    ...
    if (dom.lcd) {
      ...
      if (nativeColorActive) {
        ...
        dom.lcd.title = canToggleOelColor(activeClip)
    ...
    if (dom.lcdVideo && dom.lcdVideo.dataset) {
      ...
      dom.lcdVideo.dataset.pvfdRacingColor = toggleColorActive ? "on" : "off";
    }
  }
  ...
  function getPvfdCinemaEl() {
    ...
    pvfdCinemaEl.querySelectorAll("[data-pvfd-cinema-rail]").forEach((btn) => {
      btn.addEventListener("click", (e) => {
        e.preventDefault(); e.stopPropagation();
        setPvfdCinemaPanel(btn.getAttribute("data-pvfd-cinema-rail"));
      });
    });
  ...
  // Click slides out the keys - QUEUE / TINT - and a key opens its panel
  // OVER THE ALBUM ART, closed by the X. Lyrics never stop; the panel covers
  // art only and lives inside the art frame, so it tracks the art's box for
  // free. Both panels are functional: the queue lists what is coming, TINT
  ...
  function tickPvfdCinemaAmbient() {
    ...
    if (upnext) {
      ...
      if (show !== pvfdUpnextShown) {
        ...
        if (show) {
          const t = cinema.querySelector("[data-pvfd-cinema='upnext-title']");
          if (t) t.textContent = nextTitle.toUpperCase();
        }
  ...
  function reconcileLibraryFullscreenState() {
    ...
    const geometryFallback = !mainViewInert && !!nav &&
      nav.getBoundingClientRect().width >= window.innerWidth * 0.95;
