...
(function PioneerVFD() {
  ...
  // imported custom clip. Everything else is always VFD-tinted.
  function isColorCapableClip(clip) {
  ...
  function syncOelColorModeAttributes() {
    ...
    const racingColorActive = isColorCapableClip(activeClip) && racingColorEnabled;
    ...
    if (dom.lcd) {
      ...
      dom.lcd.title = isColorCapableClip(activeClip)
  ...
  function getClipByStorageId(key) {
    ...
    const idx = clips.findIndex((clip, clipIndex) => clipStorageId(clip, clipIndex) === key);
    return idx >= 0 ? clips[idx] : null;
  }
  ...
  function toggleRacingColorFromOel() {
    if (!isColorCapableClip(getActiveOelClip())) return;
